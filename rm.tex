...
\]
Hedge P\&L is then computed as $\mathrm{PnL}_t = h^\ast \Delta X_t$ and added to each EBIT series. Economically, the firm is harmed by rising copper prices through higher input costs, so taking a \emph{long} position in the hedge instrument provides positive hedge P\&L when copper prices rise. VaR and ETL are computed by historical simulation from the empirical EBIT distribution.
...
\subsection{Implementation note: synthetic derivative construction in Parts 5 and 6}
Two slightly different constructions are used in the code. In Part 5, the derivative \emph{changes} are generated as a linear combination
\[
\Delta D_t = \rho \, \Delta CM_t + \sqrt{1-\rho^2}\,\varepsilon_t,
\]
with $\varepsilon_t$ drawn from a normal distribution scaled to the volatility of $\Delta CM_t$. Because the sample is finite and the noise is not explicitly orthogonalized to $\Delta CM_t$, the realized \emph{sample} correlation can deviate from the target; in the reported run, the target is $0.75$ but the realized sample correlation equals $0.717$.

In Part 6, the derivative changes are generated using an orthogonalization step that removes the projection of the random noise on standardized cost changes. This construction ensures that the realized \emph{sample} correlation matches the target correlation levels (0.60, 0.75, 0.90) much more tightly, which is preferable for a clean correlation sensitivity analysis.
...
\subsection{Hedge using a synthetic derivative (target corr $0.75$, realized sample corr $0.717$)}
A synthetic derivative instrument is constructed to target correlation $0.75$ with $\Delta CM$, but in the reported run the realized sample correlation between $\Delta CM$ and $\Delta D$ equals $0.717$. The estimated hedge quantity is
\[
h^\ast_{\text{deriv}} = 24{,}360.62~\text{tons},
\qquad
N \approx \frac{24{,}360.62}{11.3398} \approx 2{,}148~\text{contracts (futures-equivalent)}.
\]
Table~\ref{tab:p5_deriv} reports the corresponding VaR/ETL results.

\begin{table}[h!]
\centering
\caption{Part 5B: EBIT risk (USD millions), unhedged vs.\ hedged with synthetic derivative (target corr $0.75$, realized sample corr $0.717$).}
\label{tab:p5_deriv}
\begin{tabular}{llccc}
\toprule
Case & Strategy & $\alpha$ & VaR$_\alpha$ & ETL$_\alpha$ \\
...
b) $\rho=0.74$ & unhedged & 0.05 & -38.2 & -53.9 \\
b) $\rho=0.92$ & unhedged & 0.05 & 28.3 & 20.3 \\
a) fixed sales & hedged (deriv corr$\sim$0.75) & 0.05 & 28.9 & 13.5 \\
b) $\rho=0.74$ & hedged (deriv corr$\sim$0.75) & 0.05 & -31.1 & -57.0 \\
b) $\rho=0.92$ & hedged (deriv corr$\sim$0.75) & 0.05 & 20.8 & 8.70 \\
\bottomrule
\end{tabular}
\end{table}

Relative to the futures hedge, the synthetic derivative hedge behaves differently. In the fixed-sales case, VaR and ETL increase slightly (from $28.6$ to $28.9$ and from $12.6$ to $13.5$), indicating a modest improvement in the left tail of EBIT. In the $\rho=0.74$ operating case, the derivative hedge improves VaR substantially (from $-38.2$ to $-31.1$), but ETL becomes more negative (from $-53.9$ to $-57.0$), which means that extremely adverse outcomes in the far tail remain severe and can even worsen. In the high revenue--commodity correlation case ($\rho=0.92$), hedging lowers both VaR and ETL, reinforcing the interpretation that the firm already has a strong natural hedge through revenues and that financial hedging can remove beneficial co-movement between revenues and costs.
...
For hedging using actual copper futures with regard to FX risk, we have
\[
h_{fut}^{\ast,\text{EUR}} = 37{,}305.18, \qquad N \approx 3290\ \text{contracts}
\]

Table \ref{tab:p5_fut_eur} compares VaR$_{0.05}$ and ETL$_{0.05}$ for unhedged and futures-hedged EBIT.
The transition from table \ref{tab:p5_fut} (USD) to table \ref{tab:p5_fut_eur} (EUR) highlights a fundamental shift in the firm’s risk profile driven by the introduction of currency volatility as a risk multiplier. In the USD-based results, the partial pass-through scenario ($\rho = 0.74$) yields a negative VaR, suggesting that pricing power acts as a near-perfect natural hedge against copper fluctuations in a single-currency environment. On the other hand, results for EU-based company show this same scenario resulting in a significantly positive VaR, as the double volatility of independent copper prices and USD/EUR exchange rate movements breaks the previous offset. 
Furthermore, the absolute magnitude of risk in table \ref{tab:p5_fut_eur} is much higher due to the nominal inflation of revenues and costs in Euro terms; while operating leverage improves average EBIT, it simultaneously amplifies the absolute scale of potential losses during tail events. Finally, the copper hedge appears less effective in this table because standard futures only mitigate the commodity price component, leaving the substantial currency-driven basis risk entirely unhedged.

\begin{table}[h!]
\centering
\caption{EBIT risk (EUR millions), unhedged vs.\ hedged with copper futures.}
\label{tab:p5_fut_eur}
\begin{tabular}{llccc}
\toprule
Case & Strategy & $\alpha$ & VaR$_\alpha$ & ETL$_\alpha$ \\
\midrule
fixed sales & unhedged & 0.05 & 13.5 & -8.15 \\
$\rho=0.74$ & unhedged & 0.05 & 59.8 & 43.3 \\
$\rho=0.92$ & unhedged & 0.05 & 147 & 137 \\
fixed sales & hedged (copper futures) & 0.05 & 12.6 & -9.53 \\
$\rho=0.74$ & hedged (copper futures) & 0.05 & 56.1 & 33.2 \\
$\rho=0.92$ & hedged (copper futures) & 0.05 & 141 & 131 \\
\bottomrule
\end{tabular}
\end{table}

For hedging using a synthetic derivative with corr $\approx 0.75$ to costs with regard to FX risk, we have
\[
h_{deriv}^{\ast,\text{EUR}} = 29{,}050.61 , \qquad N \approx 2562\ \text{contracts}
\]

The transition from the synthetic derivative analysis in the USD environment (table \ref{tab:p5_deriv}) to the EUR environment (table \ref{tab:p5_deriv_eur}) further illustrates the complicating role of exchange rate volatility. In table \ref{tab:p5_deriv}, the synthetic derivative is highly effective at stabilizing EBIT, particularly in the partial pass-through scenario ($\rho = 0.74$), where the VaR remains deeply negative. This confirms that when copper risk is the primary concern, even a moderately correlated derivative can maintain the efficiency of the firm's natural hedge.
However, in table \ref{tab:p5_deriv_eur}, the same derivative strategy yields significantly higher risk metrics across all cases. For the partial pass-through regime, the VaR shifts from the USD-based gain to a greater loss. This divergence stems from the fact that the synthetic derivative, much like the standard futures contract, is modeled to track the commodity price component rather than the combined commodity-FX cost fluctuation. 
While the derivative successfully lowers the ETL in the strong pass-through case ($\rho = 0.92$) in table \ref{tab:p5_deriv_eur}, the remaining tail risk is vastly larger than the corresponding USD tail risk. These results demonstrate that while synthetic instruments provide measurable protection, their effectiveness is inherently diluted in the EUR regime where the firm must contend with the unhedged variance of the USD/EUR exchange rate.\\

\begin{table}[h!]
\centering
\caption{Part 5B: EBIT risk (EUR millions), unhedged vs.\ hedged with synthetic derivative (corr $\approx 0.75$).}
\label{tab:p5_deriv_eur}
\begin{tabular}{llccc}
\toprule
Case & Strategy & $\alpha$ & VaR$_\alpha$ & ETL$_\alpha$ \\
\midrule
...
$\rho=0.74$ & unhedged & 0.05 & 59.8 & 43.3 \\
$\rho=0.92$ & unhedged & 0.05 & 147 & 137 \\
fixed sales & hedged (deriv corr$\sim$0.75) & 0.05 & 13.6 & -9.81 \\
$\rho=0.74$ & hedged (deriv corr$\sim$0.75) & 0.05 & 57.1 & 32.6 \\
$\rho=0.92$ & hedged (deriv corr$\sim$0.75) & 0.05 & 142 & 125 \\
\bottomrule
\end{tabular}
\end{table}


Table~\ref{tab:p6_eff_eur} reports the resulting hedge ratios and hedge effectiveness.\\

\begin{table}[h!]
\centering
\caption{Part 6: Hedge effectiveness on material-cost changes.}
\label{tab:p6_eff_eur}
\begin{tabular}{cccc}
\toprule
$\rho_{\text{deriv}}$ & Sample corr & $h^\ast$ (tons) & Hedge effectiveness \\
\midrule
0.60 & 0.60 & 32{,}295 & 0.360 \\
0.75 & 0.75 & 40{,}368 & 0.563 \\
0.90 & 0.90 & 48{,}442 & 0.810 \\
\bottomrule
\end{tabular}
\end{table}


Table~\ref{tab:p6_risk_eur} reports EBIT VaR/ETL for each correlation level.
The analysis across varying derivative correlations ($\rho_{deriv} = 0.60, 0.75, 0.90$) reveals a significant divergence between currency regimes.
Table \ref{tab:p6_risk_eur} (EUR) presents a much more resistant risk profile than table \ref{tab:p6_risk}. While increasing the derivative correlation does reduce the ETL for the $\rho = 0.92$ case, the absolute magnitude of the risk remains nearly twenty times higher than in the USD counterpart. Even with a near-perfect commodity correlation of $0.90$, the firm's $VaR_{0.05}$ stays as high as $137$ million EUR. 
This persistency of risk in the EUR model is a direct result of the unhedged USD/EUR exchange rate variance. Because the derivative only offsets the price movements of copper in USD terms, it cannot counteract the "volatility multiplier" effect of a fluctuating Euro. Consequently, table \ref{tab:p6_risk_eur} demonstrates that as the nominal scale of the business increases in EUR terms, the marginal benefit of increasing commodity-derivative precision diminishes unless a corresponding currency hedge is also implemented.

\begin{table}[h!]
\centering
\caption{EBIT risk (EUR millions) across derivative correlations.}
\label{tab:p6_risk_eur}
\begin{tabular}{llccc}
\toprule
Case & Strategy & $\rho_{\text{deriv}}$ & VaR$_{0.05}$ & ETL$_{0.05}$ \\
\midrule
fixed sales & unhedged & -- & 13.0 & -8.15 \\
$\rho=0.74$ & unhedged & -- & 59.4 & 43.3 \\
$\rho=0.92$ & unhedged & -- & 147 & 137 \\
\midrule
fixed sales & hedged (deriv corr$\sim$0.6) & 0.60 & 16.5 & -8.52 \\
$\rho=0.74$ & hedged (deriv corr$\sim$0.6) & 0.60 & 55.4 & 33.1 \\
$\rho=0.92$ & hedged (deriv corr$\sim$0.6) & 0.60 & 139 & 129 \\
\midrule
fixed sales & hedged (deriv corr$\sim$0.75) & 0.75 & 8.48 & -5.92 \\
$\rho=0.74$ & hedged (deriv corr$\sim$0.75) & 0.75 & 62.0 & 36.7 \\
$\rho=0.92$ & hedged (deriv corr$\sim$0.75) & 0.75 & 139 & 126 \\
\midrule
fixed sales & hedged (deriv corr$\sim$0.9) & 0.90 & 13.8 & -9.14 \\
$\rho=0.74$ & hedged (deriv corr$\sim$0.9) & 0.90 & 65.7 & 27.6 \\
$\rho=0.92$ & hedged (deriv corr$\sim$0.9) & 0.90 & 137 & 123 \\
\bottomrule
\end{tabular}
\end{table}
...
The results of the simulation demonstrate the effectiveness of fixing financial costs alongside operating costs. Table \ref{tab:combined_risk} summarizes the Value at Risk (VaR) and Expected Tail Loss (ETL) for Net Income across all three sales regimes ($a$, $b_1$, $b_2$) at the 5\% level.
...
In the partial pass-through regime, the risk profile is heavily influenced by the nominal scale of EUR-denominated operations.
The VaR remains relatively stable between the floating and combined strategies (approximately 53M EUR). This indicates that while the IRS stabilizes the interest expense, the dominant source of volatility in this regime remains the residual currency and commodity basis risk.
The primary benefit of the IRS is the elimination of "interest rate shock" risk, which ensures that even during periods of tight monetary policy, the firm's debt service obligations remain constant.
...
This section extends the baseline analysis to a setting in which the firm is exposed to two traded commodities. At current prices, 57\% of the sales price is linked to copper, while an additional 15\% is linked to aluminium. Aluminium price data were obtained from Yahoo Finance, where aluminium is quoted in USD per metric ton. Consequently, no additional unit conversion was required when constructing the aluminium material cost and futures price series.
Other variable material costs represent 5\% of sales, with all remaining profit and loss components unchanged relative to the initial specification.

\subsection{Commodity Price Dynamics and Cost Correlation}

Monthly spot price data are used for both commodities over the common sample period from November 2010 to June 2025. Futures price data for copper were obtained at a monthly frequency covering the period from February 2006 to December 2025. In contrast, aluminium futures prices were sourced at a daily frequency from May 2014 to November 2025. To ensure consistency with the analysis framework, all series were aligned to a common monthly frequency using end-of-period observations.
Material cost time series are constructed by scaling commodity prices such that their levels match the respective cost shares at current prices.
...
\begin{table}[h]
\centering
\caption{Unhedged EBIT risk measures in the two-commodity case}
\begin{tabular}{lcc}
\hline
Scenario & VaR$_{5\%}$ (USD million) & ETL$_{5\%}$ (USD million) \\
\hline
Fixed sales & --47.7 & --76.4 \\
Sales correlation $\rho=0.74$ & --367.0 & --406.0 \\
Sales correlation $\rho=0.92$ & --244.0 & --281.0 \\
\hline
\end{tabular}
\end{table}
...
\[
h_{\text{Copper}} = 35237.46  \quad \text{and} \quad h_{\text{Aluminium}} = 76410.75 
\]
reflecting differences in price scales and volatilities across the two futures instruments, rather than hedging intensity per se.
...
The introduction of the two-instrument hedging strategy alters the distribution of EBIT outcomes, with its effectiveness depending on the degree of sales pass-through. In the fixed-sales scenario, the 5\% EBIT VaR improves from USD --47.7 million in the unhedged case to USD --53.4 million after hedging, while the corresponding ETL improves to USD --88.1 million. Hedging effects remain economically meaningful under both sales pass-through scenarios, although absolute risk levels remain higher due to the positive co-movement between revenues and commodity prices.
\begin{table}[h]
\centering
\caption{Impact of two-commodity hedging on EBIT risk}
\begin{tabular}{lcccc}
\hline
Scenario & VaR$_{5\%}$ unhedged & VaR$_{5\%}$ hedged & ETL$_{5\%}$ unhedged & ETL$_{5\%}$ hedged \\
\hline
Fixed sales & --47.7 & --53.4 & --76.4 & --88.1 \\
Sales corr. $\rho=0.74$ & --367.0 & --367.0 & --406.0 & --412.0 \\
Sales corr. $\rho=0.92$ & --244.0 & --245.0 & --281.0 & --282.0 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Sensitivity of EBIT risk to derivative--cost correlation}
\begin{tabular}{lcc}
\hline
Derivative--cost correlation & VaR$_{5\%}$ (USD million) & ETL$_{5\%}$ (USD million) \\
\hline
0.60 & --62.0 & --91.5 \\
0.75 & --50.0 & --85.5 \\
0.90 & --53.6 & --84.7 \\
